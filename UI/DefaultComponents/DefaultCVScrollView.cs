...
namespace CP_SDK.UI.DefaultComponents
{
    ...
    public class DefaultCVScrollView : Components.CVScrollView
    {
        ...
        private float                       m_Smooth            = 8.0f;
        private float                       m_DestinationPos;
        private float                       m_ScrollBarWidth    = 6.0f;
        private float                       m_LastContentSize   = -1.0f;
        private bool                        m_ContentSizeChanged;

        private event Action<float>         m_ScrollPositionChangedEvent;
        ...
        private void Update()
        {
            if (m_ContentSizeChanged)
            {
                ...
                RefreshScrollButtons();
                m_VScrollIndicator.NormalizedPageHeight = m_ViewPort.rect.height / m_VScrollViewContent.sizeDelta.y;
            ...
            m_ScrollPositionChangedEvent?.Invoke(l_VPos);
        ...
        {
            if (Math.Abs(m_LastContentSize - p_ContentSize) < 0.01f)
                return this;
        ...
        public override Components.CVScrollView ScrollTo(float p_TargetPosition, bool p_Animated)
        {
            ...
            if (!p_Animated)
            {
                ...
                m_ScrollPositionChangedEvent?.Invoke(m_DestinationPos);
            }
