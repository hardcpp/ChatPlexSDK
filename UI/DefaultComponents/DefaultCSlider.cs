...
namespace CP_SDK.UI.DefaultComponents
{
    ...
    public class DefaultCSlider : Components.CSlider, IBeginDragHandler, IEventSystemHandler, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
    {
        ...
        private void UpdateDrag(PointerEventData p_EventData)
        {
            ...
            var l_HandleRect            = l_HandleRectTransform.rect;
            var l_SlidingRect           = m_SlidingArea.rect;

            var l_Point = l_LocalPoint - l_SlidingRect.position - new Vector2(l_HandleRect.width * 0.5f, 0f) - (l_HandleRect.size - l_HandleRectTransform.sizeDelta) * 0.5f;
            var l_Value = l_SlidingRect.width * (1f - m_HandleSize / l_SlidingRect.width);
        ...
        public void SetNormalizedValue(float p_Value, bool p_Notify = true)
        {
