    ...
    public sealed class TopNavigationView : ViewController<TopNavigationView>
    {
        ...
        protected override sealed void OnViewCreation()
        {
            XUIHLayout.Make(
                ...
                .OnReady((x) => {
                    x.LElement.flexibleWidth = 5000.0f;
                    x.CSizeFitter.enabled = false;
                    x.HOrVLayoutGroup.childForceExpandHeight = true;
                })
            )
            .SetPadding(0).SetSpacing(0)
            .OnReady((x) => {
                x.CSizeFitter.horizontalFit = UnityEngine.UI.ContentSizeFitter.FitMode.Unconstrained;
                x.CSizeFitter.enabled = false;
