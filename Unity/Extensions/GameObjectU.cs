...
namespace CP_SDK.Unity.Extensions
{
    ...
    public static class GameObjectU
    {
        ...
        public static void ChangerLayerRecursive(GameObject p_This, int p_Layer)
        {
            if (!p_This)
                return;
        ...
        /// <returns></returns>
        public static GameObject FindCommonRoot(GameObject p_Left, GameObject p_Right)
        {
            if (!p_Left || !p_Right)
                return null;

            var l_LeftTransform = p_Left.transform;
            while (l_LeftTransform)
            {
                var l_RightTransform = p_Right.transform;
                while (l_RightTransform)
                {
        ...
        /// </summary>
        /// <param name="p_This">Instance</param>
        public static void DestroyChilds(GameObject p_This)
        {
            if (!p_This)
                return;
